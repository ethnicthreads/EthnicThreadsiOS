...
#define MENU_DEFAULT_SLIDE_OFFSET 55
...
static SlideNavigationController *instance;
...
- (BOOL)shouldDisplayMenu:(Menu)menu forViewController:(UIViewController *)vc
{
    if (menu == MenuRight)
    {
        if ([vc respondsToSelector:@selector(slideNavigationControllerShouldSwipeRightMenu)] &&
            [(UIViewController<SlideNavigationControllerDelegate> *)vc slideNavigationControllerShouldSwipeRightMenu])
        {
            return YES;
        }
    }
    if (menu == MenuLeft)
    {
        if ([vc respondsToSelector:@selector(slideNavigationControllerShouldSwipeLeftMenu)] &&
            [(UIViewController<SlideNavigationControllerDelegate> *)vc slideNavigationControllerShouldSwipeLeftMenu])
        {
            return YES;
        }
    }
...
{
    [self.menuRevealAnimator clearAnimator];
