...
- (CGRect)getKeyboardRect {
    if (_keyboardVisible) return _keyboardRect;
    return CGRectZero;
}
#pragma mark - Responders, events

- (void) touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event {
    UIView *inputview = [self findFirstResponderBeneathView:self];
    if (inputview != nil) {
        [inputview resignFirstResponder];
        if (self.tpDelegate && [self.tpDelegate respondsToSelector:@selector(tpKeyboardDidFinishEditing:)]) {
            [self.tpDelegate tpKeyboardDidFinishEditing:inputview];
        }
    }
    else {
        if (self.tpDelegate && [self.tpDelegate respondsToSelector:@selector(tpKeyboardDidTouchOnView:)]) {
            [self.tpDelegate tpKeyboardDidTouchOnView:self];
        }
    }
...
- (void)keyboardWillShow:(NSNotification*)notification {
    ...
    if (!_priorInsetSaved) {
        ...
        _priorContentOffSet = self.contentOffset;
...
- (void)keyboardWillHide:(NSNotification*)notification {
    ...
    [self setContentOffset:_priorContentOffSet animated:YES];
    [self setScrollIndicatorInsets:self.contentInset];
    _priorInsetSaved = NO;
    [UIView commitAnimations];
}
...
-(BOOL)textFieldShouldReturn:(UITextField *)textField {
    if ( ![self focusNextTextField] ) {
        ...
        if (self.tpDelegate && [self.tpDelegate respondsToSelector:@selector(tpKeyboardDidFinishEditing:)]) {
            [self.tpDelegate tpKeyboardDidFinishEditing:textField];
        }
    }
    return YES;
}
