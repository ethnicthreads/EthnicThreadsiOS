...
- (void)setupAutocompleteTextField
{
    ...
    self.showAutocompleteButton = NO;
}
...
- (void)refreshAutocompleteText
{
    if (!self.autocompleteDisabled)
    {
        ...
        {
            NSArray *array = [self.autocompleteDataSource resourceForTextField:self];
            
            self.autocompleteString = @"";
            NSString *stringToLookFor;
            NSArray *componentsString = [self.text componentsSeparatedByString:@","];
//            NSString *prefixLastComponent = [componentsString.lastObject stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]];
            stringToLookFor = componentsString.lastObject;
            
            for (NSString *stringFromReference in array)
            {
                NSString *stringToCompare = stringFromReference;
                
                if ([stringToCompare hasPrefix:stringToLookFor])
                {
                    self.autocompleteString = [stringFromReference stringByReplacingCharactersInRange:[stringToCompare rangeOfString:stringToLookFor] withString:@""];
                }
