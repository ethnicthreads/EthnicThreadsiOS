...
- (void)setCancelButtonTarget:(id)target sel:(SEL)sel
{
    [btnCancel removeTarget:target action:NULL forControlEvents:UIControlEventTouchUpInside];
    [btnCancel addTarget:target action:sel forControlEvents:UIControlEventTouchUpInside];
    btnCancel.hidden = NO;
}
...
- (void)layoutSubviews {
    ...
    cancelButtonF.origin.y = yPos + detailsLabel.frame.size.height + 10;
    cancelButtonF.size = CGSizeMake(80, 40);
    cancelButtonF.origin.x = roundf((bounds.size.width - cancelButtonF.size.width) / 2) + xPos;
    btnCancel.frame = cancelButtonF;
